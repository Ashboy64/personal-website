...
You can find the source code for the interpreter [here](https://github.com/Ashboy64/rust-lambda-calc).
...
- The main potential benefir of call-by-name is that we might not have to evaluate expressions that we do not end up using. However, call-by-name also requires sharing to be practical because otherwise we might be doing a lot of deep copies when substituting a formal parameter for an expression.
- The main drawback of call-by-name is that it can lead to execution order / program behavior that is unintuitive to the programmer. 
