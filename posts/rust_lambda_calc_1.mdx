...
However, a key issue is that this grammar is _right associative_, not left associative as we had wanted. I tried quite hard to come up with a grammar that was not left-recursive but that was left-associative, and was not able to find one. In fact, I have a feeling that this might be impossible. To see this, consider the string $v_1 v_2 v_3 ... v_n$, which we want to parse as $(((v_1 v_2) v_3) ... v_n)$. Say that there is a non-left recursive grammar $G$ that does this, and that $G$ contains exactly $n-2$ nonterminals. Then the parse tree looks something like:
...
def main_functionality():
    While the root is a function app and we made a change on the last iter:
        ...
        If it is a lambda abstraction, perform substitution.
